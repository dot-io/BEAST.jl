...
function blockassembler(biop::IntegralOperator, tfs::Space, bfs::Space; quadstrat=defaultquadstrat(biop, tfs, bfs), gpu=false)
    ...
    # redirect to blockassembler_gpu if flag is set to true
    if gpu
        @info "Doing GPU assembly via CUDA"
        return blockassembler_gpu(biop, tfs, bfs)
    end
    ...
    @info "doing CPU assembly, no CUDA"
