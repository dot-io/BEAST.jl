...
# ===========================================================================
# Diagnostic: atomic contention counter
# ===========================================================================
#
# Counts how many zlocal contributions land on each output entry.
# If the max count is 1, there is zero atomic contention.
# Higher counts indicate potential serialisation of atomic adds.

"""
    scatter_contention_kernel!(counts, zlocals_all, tad_flat, tad_offsets, tad_lengths,
                               bad_flat, bad_offsets, bad_lengths,
                               active_test_ids, active_trial_ids,
                               test_id_map, trial_id_map,
                               num_tshapes, num_bshapes,
...
Diagnostic kernel: for each output entry, count how many zlocal contributions
would be atomic-added. The result goes into `counts` (Int32 matrix).
"""
function scatter_contention_kernel!(
    counts::CuDeviceMatrix{Int32},
    zlocals_all::CuDeviceArray{T,3},
    tad_flat, tad_offsets, tad_lengths,
    bad_flat, bad_offsets, bad_lengths,
    active_test_ids::CuDeviceVector{Int32},
            ...
            while ti < t_len
                ...
                if m_local != Int32(0)
                    ...
                    while bi < b_len
                        ...
                        if n_local != Int32(0)
                            CUDA.@atomic counts[m_local, n_local] += Int32(1)
                        end
...
"""
    gpu_scatter_contention(zlocals_all, tad, bad, active_test_ids, active_trial_ids,
                           test_id_map, trial_id_map, num_tshapes, num_bshapes,
                           output_rows, output_cols)

Compute per-output-entry contribution counts (fan-out) for the scatter kernel.
Returns a host-side `Matrix{Int32}` where entry `(m,n)` counts how many
zlocal contributions would be atomic-added to that position.

Useful for diagnosing atomic contention: if max(counts) == 1, there is
zero contention. Higher values indicate potential serialisation.
"""
function gpu_scatter_contention(
    zlocals_all::CuArray{T,3},
    tad::FlattenedAssemblyData{T},
    bad::FlattenedAssemblyData{T},
    active_test_ids::CuVector{Int32},
    ...
    counts = CUDA.zeros(Int32, output_rows, output_cols)

    total_pairs = Int(num_test) * Int(num_trial)
    MN = Int(num_tshapes) * Int(num_bshapes)
    total_zlocals = total_pairs * MN

    kernel = @cuda launch = false scatter_contention_kernel!(
        counts, zlocals_all,
        tad.flat, tad.offsets, tad.lengths,
        bad.flat, bad.offsets, bad.lengths,
        active_test_ids, active_trial_ids,
